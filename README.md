...
NLP作业：主要完成了对于chatglm3的指令微调和对bert的分类微调。利用大模型生成外部知识库来辅助小模型做分类。利用qwen大模型完成了2万多条数据的注释，见qwen文件夹githubtrain.csv.
## 概述：
对于情感分析任务，利用data中的数据集进行训练，进行了一系列的相关实验，包括bert微调，chatglm2微调、chatglm3微调。并且基于外部知识库进行了一个创新性实验，目前该实验效果还存在优化空间，在阅读了几篇相关论文后，有了后续改进思路。
