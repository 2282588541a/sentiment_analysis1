...
这里选择了阿里云的qwen-max，通过调用官方api实现访问，选择这个模型主要因为第一，这个模型是免费的，目前参数量较大的模型部署起来很耗费GPU资源。第二，这个模型的参数量很大，官方宣称通义千问2.1（qwen-max千亿级别模型），在分析语句的时候感觉效果还不错，这里设计两套prompt，都是few-shot：
...
这里作微调的原因主要是：直接将测试集在没有正确答案的情况下送入LLM产生的结果肯定是不可信的。因此，将上一阶段训练集产生的可信知识作为训练集进行模型微调，在微调后的模型送入测试集，来保证测试集知识的可信性。（可能有疑问，测试集通过微调后的LLM产生的知识一定是可信的吗，下面会有解释）

#### 训练bert分类器：

这里总计有八个情绪，因此将多标签预测转化为多个单标签预测。

选择将原文和刚才获得的知识拼接后送入bert分类器进行二分类任务。
