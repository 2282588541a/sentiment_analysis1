...
model = AutoModel.from_pretrained("/datas/huggingface/chatglm-6b",
                                  ...
                                  device_map='auto')
model = PeftModel.from_pretrained(model,'/data/zhangxiaoming/personal/Mistral_lora/two_points/github_chatglm4_lora_chineseLove')
model = model.merge_and_unload() #合并lora权重
model.half().cuda()

model1 = AutoModel.from_pretrained("/datas/huggingface/chatglm-6b",
                                  load_in_8bit=False, 
                                  trust_remote_code=True, 
                                  device_map='auto')
model1 = PeftModel.from_pretrained(model1,'/data/zhangxiaoming/personal/Mistral_lora/github_chatglm4_lora_chinese')
model1 = model1.merge_and_unload() #合并lora权重
model1.half().cuda()
...
prompt = """情绪文本分类任务：将一段文本进行分类，判断该段文本是否属于Love类型，属于回答Love，不属于回答unLove
下面是一些范例:
性能比 获得 多种 测试 好评 -> Love
第一 眼 就 看出 这 刚 上市 长安 福特 蒙 迪 欧 — 致胜 想 不 实 车 比 网上 照片 和 电视 里 广告 更加 诱人 -> Love
xxxxxx ->
"""
points="Love"

prompt1 = """情绪文本分类任务：将一段语句进行分类，一段语句可以属于多个分类，共分为以下八个分类 Love,Joy,Anxiety,Sorrow,Expect,Hate,Surprise,Anger
...
def get_prompt1(text):
    return prompt1.replace('xxxxxx',text)

response, his = model.chat(tokenizer, get_prompt('性能比 获得 多种 测试 好评->'), history=[])
his.append(("５０ 年 遇 大雪 阻断 无数 游子 回家 归途 也 影响 正常 上下班 -> ","unLove"))
his.append(("凑巧 那天 下午 没有 开车 而 天空 飘 起 鹅毛大雪 如果 走 着 回家 说不定 就 会 变成 雪人 就 百感交集 时候 辆 熟悉 车子 停 我的 面前 不 等 车 小柯 送 你 一程 吧 -> ","unLove"))

his.append(("瞪 大 眼睛 一看 咦 这 不 可可 嘛 什么 时候 买 新车 -> ","unLove"))
his.append(("自然 不会错 过 这么 好 机会 顺手 拉开 车门 车 内 暖和 空调 让 人 倍 感 温馨 顿时 忘却 车外 寒冷 -> ","Love"))


response, his1 = model1.chat(tokenizer, get_prompt1('性能比 获得 多种 测试 好评->'), history=[])
his1.append(("５０ 年 遇 大雪 阻断 无数 游子 回家 归途 也 影响 正常 上下班 -> ","Anxiety"))
his1.append(("凑巧 那天 下午 没有 开车 而 天空 飘 起 鹅毛大雪 如果 走 着 回家 说不定 就 会 变成 雪人 就 百感交集 时候 辆 熟悉 车子 停 我的 面前 不 等 车 小柯 送 你 一程 吧 -> ","Surprise,Anxiety,Joy"))

his1.append(("瞪 大 眼睛 一看 咦 这 不 可可 嘛 什么 时候 买 新车 -> ","Surprise"))
his1.append(("自然 不会错 过 这么 好 机会 顺手 拉开 车门 车 内 暖和 空调 让 人 倍 感 温馨 顿时 忘却 车外 寒冷 -> ","Joy,Expect,Love"))     
...
for index,row in data_tmp.iterrows():
        ...
        str3=predict(str2,str4) #二分类
        str5=predict1(str2,str4)#多分类
        df.loc[index,'er']=str3
        df.loc[index,'duo']=str5  
        temp=0
        if (points in str5):
             temp+=1
        else:
        ...
        if points==str3:
             ...
             temp-=1    
        if(temp==2)      :          
            df.loc[index,points]=points   
        elif(temp==-2)      :          
            df.loc[index,points]='un'+points       
        else :
            temp=0
            for i in range (10) :
                str3=predict(str2,str4) #二分类
