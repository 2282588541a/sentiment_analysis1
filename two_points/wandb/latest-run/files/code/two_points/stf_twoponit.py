...
for index,row in dftrain.iterrows():
    str1=str(getattr(row, 'target'))
    if points in str1:
        dftrain.loc[index,'target']=points
    else:
        dftrain.loc[index,'target']="un"+points
...
for index,row in dftest.iterrows():
    str1=str(getattr(row, 'target'))
    if points in str1:
        dftest.loc[index,'target']=points
    else:
        dftest.loc[index,'target']="un"+points
...
ckpt_path = 'github_chatglm4_lora_chinese'+points


# %%
from torchkeras.kerascallbacks import WandbCallback
wandb_cb = WandbCallback(project="glm_shot"+points,
                         config=config1,
                         name=None,
                         save_code=True,
                         save_ckpt=True)
